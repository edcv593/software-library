...
def match_software(filename, dirpath):
    ...
    elif "keygen" in lower or "注册" in lower:
        category, icon_name, desc = "激活工具", "key", "注册/激活工具"
    elif "补丁" in lower or "patch" in lower:
...
.cat-chip{display:inline-flex;align-items:center;gap:3px;padding:3px 10px;border-radius:14px;font-size:11px;font-weight:500;cursor:pointer;background:var(--bg-search);border:1px solid var(--border);color:var(--text-dim);transition:all 0.15s;white-space:nowrap;}
.cat-chip:hover{border-color:var(--accent);color:var(--accent);}
.cat-chip.active{background:var(--accent);color:#fff;border-color:var(--accent);}
.cat-chip svg{width:10px;height:10px;}
...
function renderAdminSoftwareRows(keyword){
  ...
  for(const sw of ALL_DATA){
    ...
    if(sw.downloadUrl)h+='<button class="btn btn-xs btn-primary" data-name="'+esc(sw.name)+'" onclick="fetchRemoteFile(this.dataset.name)">'+svg('download',11)+' 下载留存</button>';
    h+='</div></div>';
  }
...
async function fetchRemoteFile(name){
  ...
  if(r.success){showToast('下载留存成功：'+r.filename);setTimeout(()=>location.reload(),1000);}else showToast(r.error||'远程下载失败');
}
...
class SoftwareHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _handle_remote_fetch(self):
        try:
            data=self._read_body(); name=(data.get('name') or '').strip(); url=(data.get('url') or '').strip()
            if not name or not url:
                self._serve_json({'success':False,'error':'软件名称和下载地址不能为空'}); return
            if not re.match(r'^https?://',url,re.I):
                self._serve_json({'success':False,'error':'仅支持 HTTP/HTTPS 地址'}); return
            os.makedirs(UPLOAD_DIR,exist_ok=True)
            with requests.get(url,stream=True,timeout=(15,60),allow_redirects=True,headers={'User-Agent':'Software-Library/6.0'}) as r:
                r.raise_for_status()
                length=int(r.headers.get('Content-Length','0') or 0)
                if length>MAX_DOWNLOAD_SIZE:
                    self._serve_json({'success':False,'error':'远程文件超过 2GB 限制'}); return
                filename=''
                ...
                for part in cd.split(';'):
                    if 'filename=' in part.lower():
                        ...
                        break
                if not filename: filename=os.path.basename(urllib.parse.urlparse(r.url).path) or (name+'.download')
                filename=os.path.basename(urllib.parse.unquote(filename)).strip() or (name+'.download')
                filename=re.sub(r'[\\/:*?"<>|\x00-\x1f]','_',filename)
                base,ext=os.path.splitext(filename); save_path=os.path.join(UPLOAD_DIR,filename)
                if os.path.exists(save_path): save_path=os.path.join(UPLOAD_DIR,f'{base}_{datetime.now().strftime("%Y%m%d_%H%M%S")}{ext}')
                total=0
                with open(save_path,'wb') as f:
                    for chunk in r.iter_content(chunk_size=1024*1024):
                        ...
                        if total>MAX_DOWNLOAD_SIZE:
                            ...
                            self._serve_json({'success':False,'error':'远程文件超过 2GB 限制'}); return
                        f.write(chunk)
            run_scan(); html=generate_html()
            with open(HTML_FILE,'w',encoding='utf-8') as f:f.write(html)
            self._serve_json({'success':True,'message':'下载并留存成功','filename':os.path.basename(save_path),'size':total})
        except requests.RequestException as e:
            self._serve_json({'success':False,'error':f'远程下载失败：{e}'})
        except Exception as e:
            self._serve_json({'success':False,'error':str(e)})
